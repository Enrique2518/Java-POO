...
/**
 * Esta clase oculta toda la complejidad que hay detrás de la creación de
 * ventanas,
 * lectura de las teclas de control, dibujo de primitivas gráficas, etc... La
 * mayor
 * parte del código de esta clase es algo difícil de entender sólo con los
 * conocimientos
 * de teoría de la asignatura MOO. La gracia de la programación Orientada a
 * Objetos es
 * que no necesitas entenderlo para poder usar esta clase; simplemente puedes
 * limitarte
 * a entender qué hacen sus métodos públicos y constructores. Si eres un
 * machacas
 * y quieres aprender cosas para tu propia satisfacción, siéntete libre de
 * investigar
 ...
 * (mediante las funciones escribeTexto, dibujaCirculo, etc...), se irá
 * dibujando en un lienzo oculto.
 * Una vez se ha acabado de mostrar el lienzo, se llamará al método
 * "mostrarLienzo",
 * que mostrará el lienzo en la pantalla.
 *
 * OJO! Las coordenadas que se utilizan son <b>coordenadas de pantalla</b>. Eso
 * quiere
 * decir que el origen (0,0) está en la esquina superior izquierda de la
 * ventana.
 * Coordenadas más altas de x estarán más a la derecha de la ventana, y
 * coordenadas
 * más altas de y estarán situadas más hacia abajo (estas últimas van al
 * contrario
 * de las coordenadas Y que se suelen utilizar en las gráficas matemáticas).
 *
 * @author Mario Macías http://mario.site.ac.upc.edu
 */
public class Ventana extends JFrame implements KeyListener, WindowListener {
    /**
    ...
    /**
     * Dibuja un rectángulo en pantalla, dadas las coordenadas de su esquina
     * superior izquierda,
